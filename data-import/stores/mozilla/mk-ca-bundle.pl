...
while (<TXT>) {
  ...
  if ($start_of_cert && /^CKA_VALUE MULTILINE_OCTAL/) {
    ...
    while (<TXT>) {
      last if (/^CKA_CLASS CK_OBJECT_CLASS CKO_NSS_TRUST/ or /^CKA_CLASS CK_OBJECT_CLASS CKO_NETSCAPE_TRUST/);
      chomp;
    }
    # now scan the trust part to determine how we should trust this cert
    while (<TXT>) {
      last if (/^#/);
      if (/^CKA_TRUST_([A-Z_]+)\s+CK_TRUST\s+CKT_NSS_([A-Z_]+)\s*$/ or /^CKA_TRUST_([A-Z_]+)\s+CK_TRUST\s+CKT_NETSCAPE_([A-Z_]+)\s*$/) {
